...
import copy
#from models.model import EdgeNodeMP, GNN
...
#from models.simple_velocity import MSE_loss, variance
import h5py
import os
import argparse
import pathlib
...
def train_eval_node_regressor(model, train_loader, val_loader, save_dir=None, 
    ...
    for epoch in range(num_epochs):
        ...
        val_mse, val_R2 = eval_model(model, val_loader, device)
...
def hyperparameter_search(args):
    ...
    for n_layer, d_hidden, lr in product(n_layers_list, d_hidden_list, lr_list):
        ...
        save_dir = f"/mnt/home/thuang/playground/velocity_prediction/GNN_search/hid={d_hidden}_layers={n_layer}_lr={lr:.4f}"      
        os.makedirs(save_dir, exist_ok=True)

        R2_val = run_training(hid_dim=d_hidden, n_layer=n_layer, lr=lr, save_dir=save_dir, args=args)
    ...
    results_path = os.path.join(args.output_dir, "hyperparameter_search_results.json")
...
def eval_pretrained_model(args):
    ...
    values = np.arange(args.num_clouds)
    split_train = int(0.6 * len(values))
    split_val = int(0.8 * len(values))
    train_ranks = values[:split_train]
    val_ranks = values[split_train:split_val]
    test_ranks = values[split_val:]

    _, _, test_set = standardize_dataset(PyGdataset, train_ranks, val_ranks, test_ranks)
    if args.HGNN_flag:
        _, _, test_set_c = standardize_dataset(PyGdataset_coarse, train_ranks, val_ranks, test_ranks)
        test_set_pair = PairedDataset(test_set, test_set_c)
    ...
    model.load_state_dict(torch.load(os.path.join(best_config['save_dir'], "model.pt")))
    model.to(device)
    model.eval()
    ...
    if args.HGNN_flag:
        ...
        _, R2_all = eval_model(model, test_loader, device)
...
if __name__ == "__main__":
    ...
    #     val_set_pair = PairedDataset(val_set, val_set_c)
    #     test_set_pair = PairedDataset(test_set, test_set_c)
    #     train_loader, val_loader, _ = build_dataloaders(train_set_pair, val_set_pair, test_set_pair, 
    #                                                     batch_size=1, shuffle=True)
    # else:
    #     train_loader, val_loader, test_loader = build_dataloaders(train_set, val_set, test_set, batch_size=1, shuffle=True)
    # ##Training
    # device = "cuda" if torch.cuda.is_available() else "cpu"
    # model = VelocityGNN(node_dim=3, 
    #             d_hidden=args.hid_dim, 
    #             message_passing_steps=args.n_layer, activation='relu'
    #             )
    # model = model.to(device)
    # print(count_parameters(model))

    # save_dir = f"/mnt/home/thuang/playground/velocity_prediction/GNN_hid={args.hid_dim}_layers={args.n_layer}"
    # os.makedirs(save_dir, exist_ok=True)

    # if args.HGNN_flag:
    #     train_loss, val_loss, R2_all = train_eval_node_regressor_hierarchical(model, train_loader, val_loader, 
    #                                                                 num_epochs=300, lr=1e-3,
    #                                                                 device=device, save_dir=save_dir)
    # else:
    #     train_loss, val_loss, R2_all = train_eval_node_regressor(model, train_loader, val_loader, 
